...
class Upgrade:
	...
	def create_items(self):
		...
		for index, item in enumerate(range(self.attribute_number)):
			...
			item = AttributeItem(left,top,self.width, self.height, index, self.font)
	...
	def display(self):
		...
		for index, item in enumerate(self.item_list):
			...
			value = self.player.stats[f'{name}']
			max_value = self.player.max_stats[f'{name}']
			cost = self.player.upgrade_cost[f'{name}']
...
class AttributeItem:
	...
	def display_bar(self, surface, value, max_value, selected):
		...
		value_rect = pygame.Rect(bottom[0] - 15, bottom[1] - relative_number, 30, 10)
	...
	def trigger(self, player):
		...
		if player.stats[upgrade_attribute] != player.max_stats[upgrade_attribute]:
			if player.exp >= player.upgrade_cost[upgrade_attribute]:
				...
				player.stats[upgrade_attribute] += player.original_stats[upgrade_attribute] * 0.2
